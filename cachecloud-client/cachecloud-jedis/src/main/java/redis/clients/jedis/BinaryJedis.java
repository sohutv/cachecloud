...
public class BinaryJedis implements BasicCommands, BinaryJedisCommands, MultiKeyBinaryCommands,
  ...
  public BinaryJedis(final HostAndPort hp, final StatsCollector statsCollector) {
    this(hp.getHost(), hp.getPort(), statsCollector);
  }
  ...
  public BinaryJedis(final String host, final int port, final StatsCollector statsCollector) {
    client = new Client(host, port, statsCollector);
  }
  ...
  public BinaryJedis(final String host, final int port, final StatsCollector statsCollector, final int connectionTimeout,
       final int soTimeout) {
     client = new Client(host, port, statsCollector);
     client.setConnectionTimeout(connectionTimeout);
     client.setSoTimeout(soTimeout);
  ...
  public BinaryJedis(final String host, final int port, final int connectionTimeout,
    ...
    public BinaryJedis(final String host, final int port, final int connectionTimeout,
                       final int soTimeout, final boolean ssl, final SSLSocketFactory sslSocketFactory,
                       final SSLParameters sslParameters, final HostnameVerifier hostnameVerifier, final StatsCollector statsCollector) {
        client = new Client(host, port, ssl, sslSocketFactory, sslParameters, hostnameVerifier, statsCollector);
        client.setConnectionTimeout(connectionTimeout);
        client.setSoTimeout(soTimeout);
    }
  ...
  @Override
  public List<byte[]> latencyLatestBinary() {
    client.latencyLatest();
    return client.getBinaryMultiBulkReply();
  }

  @Override
  public List<byte[]> latencyHistoryBinary(byte[] event) {
    client.latencyHistory(event);
    return client.getBinaryMultiBulkReply();
  }

  @Override
  public Long latencyResetBinary(byte[]... events) {
    client.latencyReset(events);
    return client.getIntegerReply();
  }

  @Override
  public byte[] latencyDoctorBinary() {
    checkIsInMultiOrPipeline();
