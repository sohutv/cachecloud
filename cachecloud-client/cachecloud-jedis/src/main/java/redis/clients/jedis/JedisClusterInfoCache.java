...
public class JedisClusterInfoCache {
  ...
  public JedisClusterInfoCache(final GenericObjectPoolConfig poolConfig, final StatsCollector statsCollector, final int connectionTimeout,
                               final int soTimeout, final String user, final String password, final String clientName,
                               boolean ssl, SSLSocketFactory sslSocketFactory, SSLParameters sslParameters,
                               HostnameVerifier hostnameVerifier, JedisClusterHostAndPortMap hostAndPortMap) {
    this.poolConfig = poolConfig;
  ...
  public JedisPool setupNodeIfNotExist(HostAndPort node) {
    ...
    try {
      ...
      JedisPool nodePool = new JedisPool(poolConfig, statsCollector, node.getHost(), node.getPort(),
  ...
  public void removeNodeIfExist(HostAndPort node){
     w.lock();
     try {
       String nodeKey = getNodeKey(node);
       JedisPool existingPool = nodes.get(nodeKey);
  ...
  public boolean isInSlots(JedisPool jedisPool){
    r.lock();
    try {
      return slots.values().contains(jedisPool);
    } finally {
      r.unlock();
    }
  }
