...
public class BinaryClient extends Connection {
  ...
  public BinaryClient(final String host, final int port, final StatsCollector statsCollector) {
    super(host, port, statsCollector);
  }
  ...
  public BinaryClient(final String host, final int port, final boolean ssl,
                      final SSLSocketFactory sslSocketFactory, final SSLParameters sslParameters,
                      final HostnameVerifier hostnameVerifier, final StatsCollector statsCollector) {
    super(host, port, ssl, sslSocketFactory, sslParameters, hostnameVerifier, statsCollector);
  }
  ...
  public void zcount(final byte[] key, final String min, final String max) {
    sendCommand(ZCOUNT, key, min.getBytes(Charset.forName("UTF-8")), max.getBytes(Charset.forName("UTF-8")));
  }
  ...
  public void zrangeByScore(final byte[] key, final String min, final String max) {
    sendCommand(ZRANGEBYSCORE, key, min.getBytes(Charset.forName("UTF-8")), max.getBytes(Charset.forName("UTF-8")));
  }
  ...
  public void zrevrangeByScore(final byte[] key, final String max, final String min) {
    sendCommand(ZREVRANGEBYSCORE, key, max.getBytes(Charset.forName("UTF-8")), min.getBytes(Charset.forName("UTF-8")));
  }
  ...
  public void zrangeByScore(final byte[] key, final String min, final String max, final int offset,
    ...
    sendCommand(ZRANGEBYSCORE, key, min.getBytes(Charset.forName("UTF-8")), max.getBytes(Charset.forName("UTF-8")), LIMIT.raw, toByteArray(offset),
      toByteArray(count));
  }
  ...
  public void zrevrangeByScore(final byte[] key, final String max, final String min,
    ...
    sendCommand(ZREVRANGEBYSCORE, key, max.getBytes(Charset.forName("UTF-8")), min.getBytes(Charset.forName("UTF-8")), LIMIT.raw,
  ...
  public void zrevrangeByScoreWithScores(final byte[] key, final String max, final String min) {
    sendCommand(ZREVRANGEBYSCORE, key, max.getBytes(Charset.forName("UTF-8")), min.getBytes(Charset.forName("UTF-8")), WITHSCORES.raw);
  }
  ...
  public void memoryStats(){
    sendCommand(MEMORY, Keyword.STATS.raw);
  }

  public void memoryPurge(){
    sendCommand(MEMORY, Keyword.PURGE.raw);
  }

  public void latencyLatest() {
    sendCommand(LATENCY, Keyword.LATEST.raw);
  }

  public void latencyHistory(byte[] event){
    sendCommand(LATENCY, Keyword.HISTORY.raw, event);
  }

  public void latencyReset(byte[]... events){
    sendCommand(LATENCY, joinParameters(Keyword.RESET.raw, events));
  }

  public void latencyDoctor(){
    sendCommand(LATENCY, Keyword.DOCTOR.raw);
  }
  ...
  public void clientGetRedir(){
    sendCommand(CLIENT, Keyword.GETREDIR.raw);
  }

  public void clientTracking(ClientTrackingParams.Latch latch, ClientTrackingParams params){
    sendCommand(CLIENT, params.getByteParams(Keyword.TRACKING.raw, latch.raw));
  }

  public void clientTracking(ClientTrackingParams.Latch latch){
    sendCommand(CLIENT, Keyword.TRACKING.raw, latch.raw);
  }
