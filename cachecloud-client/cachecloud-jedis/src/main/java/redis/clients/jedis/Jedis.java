...
public class Jedis extends BinaryJedis implements JedisCommands, MultiKeyCommands,
  ...
  public Jedis(final HostAndPort hp, final StatsCollector statsCollector) {
    super(hp, statsCollector);
  }
  ...
  public Jedis(final String host, final int port, final StatsCollector statsCollector) {
  ...
  public Jedis(final String host, final int port, final StatsCollector statsCollector, final int connectionTimeout, final int soTimeout) {
    super(host, port, statsCollector, connectionTimeout, soTimeout);
  }
  ...
  public Jedis(final String host, final int port, final int connectionTimeout, final int soTimeout,
    ...
    super(host, port, connectionTimeout, soTimeout, ssl, sslSocketFactory, sslParameters,
            hostnameVerifier, statsCollector);
  }
  ...
  @Override
  public String sentinelFlushConfig() {
    client.sentinel(Protocol.SENTINEL_FLUSH_CONFIG);
    return client.getStatusCodeReply();
  }
  ...
  public Long clientGetRedir() {
    checkIsInMultiOrPipeline();
    client.clientGetRedir();
    return client.getIntegerReply();
  }

  @Override
  public String clientTracking(ClientTrackingParams.Latch latch) {
    checkIsInMultiOrPipeline();
    client.clientTracking(latch);
    return client.getStatusCodeReply();
  }

  @Override
  public String clientTracking(ClientTrackingParams.Latch latch, ClientTrackingParams params) {
    checkIsInMultiOrPipeline();
    client.clientTracking(latch, params);
    return client.getStatusCodeReply();
  }
  ...
  @Override
  public List<LatencyItem> latencyLatest() {
    client.latencyLatest();
    return LatencyItem.from(client.getObjectMultiBulkReply());
  }

  @Override
  public List<LatencyHistoryItem> latencyHistory(String event) {
    client.latencyHistory(event);
    return LatencyHistoryItem.from(client.getObjectMultiBulkReply());
  }

  @Override
  public Long latencyReset(String... events) {
    client.latencyReset(events);
    return client.getIntegerReply();
  }
  ...
  @Override
  public List<Object> memoryStats() {
    client.memoryStats();
    List<Object> resps = client.getObjectMultiBulkReply();
    for(int i=0;i<resps.size();i++){
      Object obj = resps.get(i);
      if(obj instanceof byte[]){    //byte[]->String
        resps.set(i, SafeEncoder.encode((byte[])obj));
      } else if(obj instanceof List){
        List<Object> nestedList = (List<Object>)obj;
        for(int j=0;j<nestedList.size();j++){
          Object innerObj = nestedList.get(j);
          if(innerObj instanceof byte[]){
            nestedList.set(j, SafeEncoder.encode((byte[])innerObj));
          }
        }
      }
    }
