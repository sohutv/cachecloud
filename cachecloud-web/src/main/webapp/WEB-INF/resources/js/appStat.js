...
function reloadAppStatPage(appId){
	location.href = "/admin/app/index?appId=" + appId + "&tabTag=app_stat";
}

//申请键值分析
function appKeyAnalysis(appId){
	var appAnalysisReason = document.getElementById("appAnalysisReason");
	if(appAnalysisReason.value == ""){
		alert("请填写键值分析原因!");
		appAnalysisReason.focus();
		return false;
	}
	var nodeInfo = document.getElementById("nodeInfo");

	var appKeyAnalysisBtn = document.getElementById("appKeyAnalysisBtn");
	appKeyAnalysisBtn.disabled = true;
	...
	$.post(
		'/admin/app/keyAnalysis',
		{
			...
			appAnalysisReason: appAnalysisReason.value
		},
		function(data){
			if(data==1){
				alert("申请成功，请在邮件中关注申请状况.");
				$("#appKeyAnalysisInfo").html("<div class='alert alert-error' ><button class='close' data-dismiss='alert'>×</button><strong>Success!</strong>更新成功，窗口会自动关闭</div>");
				setTimeout("$('appKeyAnalysisModal').modal('hide');reloadAppStatPage("+appId+");",1000);
			}else{
				appKeyAnalysisBtn.disabled = false;
				$("#appKeyAnalysisInfo").html("<div class='alert alert-error' ><button class='close' data-dismiss='alert'>×</button><strong>Error!</strong>更新失败！</div>");
			}
		}
	);
}
