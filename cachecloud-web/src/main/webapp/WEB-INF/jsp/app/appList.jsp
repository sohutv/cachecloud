        ...
        <div class="col-md-12">
            ...
            <c:if test="${currentUser.type == 0}">
				<div style="float:right">
					<form class="form-inline" role="form" method="post" action="/admin/app/list" id="appList" name="ec">
					<div class="form-group" style="margin-right: 70px">
						<select id="userId" name="userId" class="form-control selectpicker" data-live-search="true">
							...
							<c:forEach items="${userList}" var="user">
								<option value="${user.id}" <c:if test="${user.id == appSearch.userId}">selected</c:if>>${user.chName}(${user.name})</option>
							</c:forEach>
						</select>
					</div>
				      ...
				      <div class="form-group">
						<select name="versionId" id="versionId" class="form-control select2_category">
							...
							<c:forEach items="${resourcelist}" var="version">
								<option value="${version.id}" <c:if test="${version.id == appSearch.versionId}">selected</c:if>>${version.name}</option>
							</c:forEach>
						</select>
				 	  </div>
					  <div class="form-group">
			                    ...
			                    <td>
			                    	<c:choose>
			                    		...
			                    		<c:when test="${appDetail.appDesc.status == 2 or appDetail.appDesc.status == 3 or appDetail.appDesc.status == 4}">
			                    			<a title="${appDetail.appDesc.intro}" target="_blank" href="/admin/app/index?appId=${appDetail.appDesc.appId}">${appDetail.appDesc.appId}</a>
			                    		</c:when>
			                    	</c:choose>
			                    </td>
			                    <td>
			                    	<c:choose>
			                    		...
			                    		<c:when test="${appDetail.appDesc.status == 2 or appDetail.appDesc.status == 3 or appDetail.appDesc.status == 4}">
			                    			<a target="_blank" href="/admin/app/index?appId=${appDetail.appDesc.appId}">${appDetail.appDesc.name}</a>
			                    		</c:when>
			                    	</c:choose>
			                    </td>
								 <td>
									 <c:choose>
										 <c:when test="${appDetail.appDesc.isTest == 1}">测试</c:when>
										 <c:when test="${appDetail.appDesc.isTest == 0}">正式</c:when>
									 </c:choose>
								 </td>
